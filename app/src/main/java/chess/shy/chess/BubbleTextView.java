...
public class BubbleTextView extends TextView {
    ...
    public BubbleTextView(Context context,PieceInfo pieceInfo) {
        super(context);
    ...
    private void init() {
        ...
        mBg = getResources().getDrawable(R.drawable.b);
    ...
    public void moveTo(int x, int y) {
        ...
        final int startX = getLeft();
        final int startY = getTop();
        if ((x - startX) != 0 || (y - startY) != 0) {
            mScroller.startScroll(startX, startY, x - startX, y - startY, ANIMATION_DURATION);
            postInvalidate();
        }
    }
    ...
    public void applyFromPieceInfo(PieceInfo info) {
        setText(info.title);
        if (info.team == 0) {
    ...
    void setCellLayoutPressedOrFocusedIcon() {
        if (getParent() instanceof ChessBoard) {
